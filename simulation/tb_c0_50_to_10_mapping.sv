...
module tb_c0_50_to_10_mapping;

    localparam logic [8:0]  C0_BYTE_OFFSET = 9'h05C;
    localparam logic [8:0]  C0_ADDRESS = C0_BYTE_OFFSET >> 2;
    localparam logic [31:0] C0_BASELINE = 32'h10000C20;
    localparam logic [31:0] C0_TARGET   = 32'h50000CA0;
    ...
    initial begin
        ...
        // Independently encode low=160 at [30:23], high=160 at [7:0],
        ...
        check(calculated_target == C0_TARGET,
              "independent C0 RMW result is not 0x50000CA0");
        ...
        check(final_readback[7:0] == 8'd160, "high_count decode mismatch");
        check(final_readback[30:23] == 8'd160, "low_count decode mismatch");
        ...
        $display("-------------------------+--------------------+------------+----------");
        print_result("RMW calculation", "0x50000CA0", calculated_target,
                     calculated_target == C0_TARGET);
        print_result("address conversion", "0x00000017", {23'd0, C0_ADDRESS},
                     C0_ADDRESS == 9'h017);
        print_result("byte order", "A0 0C 00 50", {actual_write_bytes[8],
                     actual_write_bytes[7], actual_write_bytes[6],
                     actual_write_bytes[5]},
                     actual_write_bytes[5] == 8'hA0 &&
                     actual_write_bytes[6] == 8'h0C &&
                     actual_write_bytes[7] == 8'h00 &&
                     actual_write_bytes[8] == 8'h50);
        ...
        print_result("D3 hold timing", "1 extra cycle", actual_d3_hold_cycles,
                     ...
                     actual_write_bytes[9] == 8'h50);
        print_result("idle timing", ">=5 cycles", actual_idle_cycles,
                     actual_idle_cycles >= 5);
        print_result("mock register write", "0x50000CA0", mock_c0,
                     mock_c0 == C0_TARGET);
        print_result("32-bit readback", "0x50000CA0", final_readback,
                     final_readback == C0_TARGET);
        print_result("high_count", "160", final_readback[7:0],
                     final_readback[7:0] == 8'd160);
        print_result("low_count", "160", final_readback[30:23],
                     final_readback[30:23] == 8'd160);
        ...
        $display("\n50 TO 10 MHZ C0 MAPPING PASSED (representation/transaction only)");
        $finish;
    end
